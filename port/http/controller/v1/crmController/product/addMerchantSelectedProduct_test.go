...
func TestAddMerchantSelectedProduct(t *testing.T) {
	testcases := []struct {
		...
		},
		{
			...
			ExpectedResponse: `{"code":"40","errors":"json: cannot unmarshal string into Go value of type product.AddMerchantProductRequest"}`,
		},
		{
			...
			WantErr:          true,
			ExpectedStatus:   http.StatusBadRequest,
			ExpectedResponse: `{"code":"40","errors":{"MerchantID":"Key: 'AddMerchantProductRequest.MerchantID' Error:Field validation for 'MerchantID' failed on the 'required' tag","ProductID":"Key: 'AddMerchantProductRequest.ProductID' Error:Field validation for 'ProductID' failed on the 'required' tag"}}`,
		},
		{
	...
	for _, tt := range testcases {
		t.Run(tt.Name, func(t *testing.T) {
			...
			req := httptest.NewRequest(http.MethodPost, "/products", bytes.NewBuffer(tt.SetupBody()))
			req = tt.SetupRequest(req)
			rr := httptest.NewRecorder()

			// Create the handler and serve the request
			handler := http.HandlerFunc(controller.AddMerchantSelectedProduct)
