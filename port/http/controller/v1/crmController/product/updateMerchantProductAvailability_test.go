...
func TestUpdateMerchantProductAvailability(t *testing.T) {
	testcases := []struct {
		...
		},
		{
			...
			ExpectedResponse: ` {"code":"40","errors":"incorrect merchant id"}`,
		},
		{
			...
			WantErr:          true,
			ExpectedStatus:   http.StatusBadRequest,
			ExpectedResponse: `{"code":"40","errors":"json: cannot unmarshal string into Go value of type product.UpdateMerchantSelectedProductAvailabilityRequest"}`,
		},
		{
			...
			WantErr:          true,
			ExpectedStatus:   http.StatusBadRequest,
			ExpectedResponse: `{"code":"40","errors":{"ProductID":"Key: 'UpdateMerchantSelectedProductAvailabilityRequest.ProductID' Error:Field validation for 'ProductID' failed on the 'required' tag"}}`,
		},
		{
			Name: "ERROR: Error get merchant selected products",
			SetupRequest: func(req *http.Request) *http.Request {
				ctx := req.Context()
				chiCtx := chi.NewRouteContext()
				chiCtx.URLParams.Add("merchantId", uuid.NewString())
	...
	for _, tt := range testcases {
		t.Run(tt.Name, func(t *testing.T) {
			...
			req := httptest.NewRequest(http.MethodPut, "/products/merchants", bytes.NewBuffer(tt.SetupBody()))
			req = tt.SetupRequest(req)
			rr := httptest.NewRecorder()

			// Create the handler and serve the request
			handler := http.HandlerFunc(controller.UpdateMerchantProductAvailability)
