...
func TestDeleteDefaultRolePermissions(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			mockSetup: func(roleSvc *mockService.IRoleService) {
				roleSvc.On("DeleteDefaultRolePermissions", mock.Anything, mock.AnythingOfType("*role.CRMUpdateDefaultRolePermissionsRequest")).
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			req := httptest.NewRequest(http.MethodDelete, "/api/v1/crm/roles/default-permissions", bytes.NewBuffer(body))
			...
			// Call handler
			controller.DeleteDefaultRolePermissions(w, req)
