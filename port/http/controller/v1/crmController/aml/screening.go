...
// @Description		Use screening to check for AML
// @ID				crm-aml-screening
...
// @Router			crm/v1/aml/screening [post]
// @Header       	all {string}  X-CRM-Key "{"key": "value"}"
func (c *CRMAmlController) Screening(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	...
	resp, err := c.amlService.Screening(ctx, &payload, provider, merchantID)
	if err != nil {
		response.SendApiResponseError(ctx, w, errors.New(response.HttpErrRequest, err))
		return
	}
