...
func TestRetryTransaction(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			disbursementID: validDisbursementID,
			setupBody: func(t *testing.T) []byte {
				payload := disbursementModel.RetryTransaction{
					DisbursementID: validDisbursementID,
				}
			...
			wantStatusCode: http.StatusBadRequest,
			wantRespBody:   `{"code":"42","errors":{"MerchantID":"Key: 'RetryTransaction.MerchantID' Error:Field validation for 'MerchantID' failed on the 'required' tag"}}`,
		},
		{
			name:           "ERROR: RetryDueToInsufficientEscrowFund service error",
			disbursementID: validDisbursementID,
			setupBody: func(t *testing.T) []byte {
				payload := disbursementModel.RetryTransaction{
					DisbursementID: validDisbursementID,
			...
			modifierMock: func() {
				svc.On(
					"RetryDueToInsufficientEscrowFund",
					constant.ValueCtxMockType(),
					mock.AnythingOfType("*disbursementModel.RetryTransaction"),
		...
		},
		{
			...
			wantRespBody:   `{"code":"00","data":{"id":"` + validDisbursementID + `"}}`,
		},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			req := httptest.NewRequest(http.MethodPost, fmt.Sprintf("/disbursements/%s/retry-transaction", test.disbursementID), bytes.NewBuffer(test.setupBody(t)))

			router := chi.NewRouter()
			router.Post("/disbursements/{id}/retry-transaction", New(svc, nil).RetryTransaction)
