...
func TestGetFlipEscrowBalance(t *testing.T) {
	testCases := []struct {
		name           string
	...
	}{
		{
			...
			setupMock: func(req *http.Request, svc *serviceMocks.IRoutingProcessorService) {
				svc.On("GetFlipEscrowBalance", mock.Anything, mock.AnythingOfType("string")).Return(&routingProcessorModelEscrowBalance.EscrowBalanceResponse{
		...
		},
		{
			...
			setupMock: func(req *http.Request, svc *serviceMocks.IRoutingProcessorService) {
				svc.On("GetFlipEscrowBalance", mock.Anything, mock.AnythingOfType("string")).Return(nil, nil)
			},
		},
	}
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			req, err := http.NewRequest("GET", "/crm/v1/balances/flip-balance", nil)
			if err != nil {
				t.Fatal(err)
			}
