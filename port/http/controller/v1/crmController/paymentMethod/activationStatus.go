...
func (h *handler) ChangeActivationStatus(w http.ResponseWriter, r *http.Request) {
	ctx, segment := otelTracer.Start(r.Context(), "port/http/controller/v1/crmController/paymentMethod/ChangeActivationStatus")
	defer segment.End()

	var (
		payload *paymentMethodModel.ChangeActivationStatusRequest
		err     error
	)
	...
	if err = h.paymentMethodSvc.ChangeActivationStatus(ctx, payload); err != nil {
