package crmRateLimiterController

import (
	"bytes"
	"context"
	"errors"
	"net/http"
	"net/http/httptest"
	"strings"
	"testing"

	"github.com/go-chi/chi/v5"
	"github.com/go-playground/validator/v10"
	"github.com/google/uuid"
	rateLimiterModel "github.com/paper-indonesia/pivot-backoffice/internal/model/rateLimiter"
	mockService "github.com/paper-indonesia/pivot-backoffice/mocks/service"
	"github.com/stretchr/testify/assert"
	"github.com/stretchr/testify/mock"
)

func TestCRMRateLimiterController_Create(t *testing.T) {
	merchantID := uuid.New().String()

	tests := []struct {
		name         string
		merchantID   string
		requestBody  string
		mockSetup    func(mockSvc *mockService.IRateLimiter)
		expectedCode int
		expectError  bool
	}{
		{
			name:       "SUCCESS: Create rate limit configuration",
			merchantID: merchantID,
			requestBody: `{
				"limit": 10,
				"burst": 5,
				"order": 1,
				"time": "MINUTE",
				"variable": "IP",
				"variableValue": "127.0.0.1",
				"variableMatchType": "EXACT",
				"httpMethod": "POST",
				"description": "Test rate limit"
			}`,
			mockSetup: func(mockSvc *mockService.IRateLimiter) {
				mockSvc.On("Create", mock.Anything, mock.MatchedBy(func(req *rateLimiterModel.CreateRateLimitConfiguration) bool {
					return req.MerchantID == merchantID && req.Limit == 10
				})).Return(&rateLimiterModel.RateLimitConfiguration{
					UUID:       uuid.New().String(),
					MerchantID: merchantID,
					Limit:      10,
				}, nil).Once()
			},
			expectedCode: http.StatusOK,
			expectError:  false,
		},
		{
			name:         "ERROR: Invalid merchant ID",
			merchantID:   "invalid-uuid",
			requestBody:  `{"limit": 10}`,
			mockSetup:    func(mockSvc *mockService.IRateLimiter) {},
			expectedCode: http.StatusBadRequest,
			expectError:  true,
		},
		{
			name:         "ERROR: Invalid JSON",
			merchantID:   merchantID,
			requestBody:  `{invalid json}`,
			mockSetup:    func(mockSvc *mockService.IRateLimiter) {},
			expectedCode: http.StatusBadRequest,
			expectError:  true,
		},
		{
			name:       "ERROR: Validation fails",
			merchantID: merchantID,
			requestBody: `{
				"limit": 0,
				"time": "INVALID"
			}`,
			mockSetup:    func(mockSvc *mockService.IRateLimiter) {},
			expectedCode: http.StatusBadRequest,
			expectError:  true,
		},
		{
			name:       "ERROR: Service create fails",
			merchantID: merchantID,
			requestBody: `{
				"limit": 10,
				"burst": 5,
				"order": 1,
				"time": "MINUTE",
				"variable": "IP",
				"variableValue": "127.0.0.1",
				"variableMatchType": "EXACT",
				"httpMethod": "POST"
			}`,
			mockSetup: func(mockSvc *mockService.IRateLimiter) {
				mockSvc.On("Create", mock.Anything, mock.AnythingOfType("*ratelimiter.CreateRateLimitConfiguration")).
					Return(nil, errors.New("service error")).Once()
			},
			expectedCode: http.StatusInternalServerError,
			expectError:  true,
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			mockSvc := mockService.NewIRateLimiter(t)
			controller := &CRMRateLimiterController{
				svc:       mockSvc,
				validator: validator.New(),
			}

			if tt.mockSetup != nil {
				tt.mockSetup(mockSvc)
			}

			req := httptest.NewRequest(http.MethodPost, "/merchants/{merchantId}/rate-limits", 
				bytes.NewBufferString(tt.requestBody))
			rctx := chi.NewRouteContext()
			rctx.URLParams.Add("merchantId", tt.merchantID)
			req = req.WithContext(context.WithValue(req.Context(), chi.RouteCtxKey, rctx))

			w := httptest.NewRecorder()
			controller.Create(w, req)

			assert.Equal(t, tt.expectedCode, w.Code)
			if tt.expectError {
				assert.Contains(t, strings.ToLower(w.Body.String()), "error")
			}

			mockSvc.AssertExpectations(t)
		})
	}
}