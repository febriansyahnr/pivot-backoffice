...
func TestBulkValidate(t *testing.T) {
	...
	handler := New(
		&config.Config{}, validator.New(), nil, service, nil, nil,
		WithLogger(nil), WithRedisClient(nil),
	)
	router := chi.NewRouter()
	router.Post("/bulk/validate", handler.BulkValidate)
	...
	tests := []struct {
		...
		},
		{
			...
			setupMock: func() {
				disbursementSvc.On(
					"BulkValidate", mock.Anything, mock.AnythingOfType("*disbursementModel.BulkPreviewRequest"), // NOSONAR
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			req := httptest.NewRequest(http.MethodPost, "/bulk/validate", body)
