...
func TestRetrySingle(t *testing.T) {
	...
	validPayload := disbursementModel.RetrySingleRequest{
	...
	testCases := []struct {
			...
			mockSetup: func(disbursementSvc *mocks.IDisbursementService) {
				disbursementSvc.
					On(
						...
						mock.AnythingOfType("*disbursementModel.RetrySingleRequest")).
					Return(nil)
			},
		...
		},
		{
			...
			mockSetup: func(disbursementSvc *mocks.IDisbursementService) {},
			setupBody: func(t *testing.T) []byte {
				payloadRequestByte, err := json.Marshal(disbursementModel.RetrySingleRequest{})
				assert.NoError(t, err)
				return payloadRequestByte
			},
			expectedStatus: http.StatusBadRequest,
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			...
			req := httptest.NewRequest(http.MethodPost, "/api/v1/disbursements/single/retry", bytes.NewBuffer(tt.setupBody(t)))
			if tt.userClaim != nil {
			...
			handler := http.HandlerFunc(mc.RetrySingle)
