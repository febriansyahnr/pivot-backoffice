...
func (c *Controller) BulkPreview(w http.ResponseWriter, r *http.Request) {

	ctx, segment := otelTracer.Start(r.Context(), "port/http/controller/v1/disbursement/BulkPreview")
	defer segment.End()
	...
	if previewResult, err := c.disbursementSvc.BulkPreview(ctx, request); err != nil {
