...
func TestCheckPin(t *testing.T) {
	...
	testCases := []struct {
		name           string
		requestBody    []byte
		mockSetup      func(userSvc *mockUser.IUserService, mockRabbit *mockRabbitMq.RabbitMQExt)
		expectedStatus int
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			...
			mockRabbitMQ := mockRabbitMq.NewRabbitMQExt(t)
			mockValidator := validator.New()

			tt.mockSetup(mockUserSvc, mockRabbitMQ)

			mc := New(mockValidator, mockUserSvc, nil, nil, nil, nil, cfg, secret, mockRabbitMQ, nil)
			...
			handler := http.HandlerFunc(mc.CheckCurrentPin)
