...
func TestUserController_UnblockUser(t *testing.T) {
	payloadReq := &userModel.UserUnblockRequest{
		Email: []string{"test@gmail.com"},
	}

	payloadRequestByte, err := json.Marshal(payloadReq)
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			...
			req := httptest.NewRequest(http.MethodPost, "/crm/v1/unblock", bytes.NewBuffer(tt.requestBody))
			rr := httptest.NewRecorder()

			// Create the handler and serve the request
			handler := http.HandlerFunc(mc.UnblockUser)
