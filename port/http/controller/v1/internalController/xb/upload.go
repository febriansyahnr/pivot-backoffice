...
func (c *InternalXbController) UploadUnderlyingDocument(w http.ResponseWriter, r *http.Request) {
	ctx, segment := otelTracer.Start(r.Context(), "port/http/controller/v1/internalController/xb/UploadUnderlyingDocument")
	defer segment.End()
	...
	if !constant.IsUnderlyingDocumentValidToUpload(request.Document.Filename) {
		response.SendOpenApiNonSnapResponseError(ctx, w, pkgErrors.New(response.HttpErrRequest, errors.New("document format is not supported")))
		return
	}
	...
	defer func() {
		metricData := &monitoring.CustomMetric{
			...
			MetricName:           constant.MetricNameXBUploadPayout,
