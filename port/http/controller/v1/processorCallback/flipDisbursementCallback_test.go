...
func TestFlipDisbursementCallback(t *testing.T) {
	tests := []struct {
		name           string
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			req, err := http.NewRequest("POST", "/callback/v1/flip/special-transfer", tc.requestBody())
			if err != nil {
				t.Fatal(err)
			}
			...
			handler := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				...
				ctrl.FlipDisbursementCallback(w, r)
			})
