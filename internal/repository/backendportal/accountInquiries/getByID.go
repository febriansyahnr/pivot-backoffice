...
func (r *AccountInquiriesRepository) GetByID(ctx context.Context, id string) (*accountInquiries.AccountInquiries, error) {
	...
	if err := r.db.GetContext(ctx, &account, query, id); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			r.logger.Info(ctx, "account inquiry not found", logger.Any("data", map[string]string{"uuid": id}))
			return nil, nil
		}
