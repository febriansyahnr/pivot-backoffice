...
func TestGetMerchantsWithoutAccount(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name: "SUCCESS: Select merchants without account",
			mockSetup: func(mysqlMock *mysqlMocks.IMySqlExt) {
				mysqlMock.On(
					"SelectContext",
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			_, err := repo.GetMerchantsWithoutAccount(context.Background(), request)
