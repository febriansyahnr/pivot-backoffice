...
func TestFindLatestByInquiryID(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name: "SUCCESS: FindLatestByInquiryID ",
			mockSetup: func(mysqlMock *mysqlMocks.IMySqlExt) {
				mysqlMock.On(
					"GetContext",
		...
		{
			name: "SUCCESS: FindLatestByInquiryID with valid metadata",
			mockSetup: func(mysqlMock *mysqlMocks.IMySqlExt) {
				mysqlMock.On(
					"GetContext",
					mock.AnythingOfType(constant.MockTypeValueContextReference),
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			_, err := repo.FindLatestByInquiryID(ctx, uuid.NewString(), uuid.NewString())
			if tc.wantErr {
				assert.Error(t, err)
			} else {
...
func TestFindLatestWithMasterByInquiryID(t *testing.T) {
	existedInquiry := &requestAccountInquiry.RequestAccountInquiryWithMaster{
		RequestAccountInquiries: requestAccountInquiry.RequestAccountInquiries{
			UUID:       uuid.NewString(),
			MerchantID: uuid.NewString(),
	...
	testCases := []struct {
		...
		{
			name: "SUCCESS: FindLatestWithMasterByInquiryID with valid metadata",
			mockSetup: func(mysqlMock *mysqlMocks.IMySqlExt) {
				mysqlMock.On(
					"GetContext",
					mock.AnythingOfType(constant.MockTypeValueContextReference),
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			repo := New(mockMysql, mockLogger)
			ctx := context.WithValue(context.Background(), mySqlExt.CtxSQLTableNameKey, tableName)
			_, err := repo.FindLatestWithMasterByInquiryID(ctx, uuid.NewString(), uuid.NewString())
			if tc.wantErr {
				assert.Error(t, err)
			} else {
				assert.NoError(t, err)
