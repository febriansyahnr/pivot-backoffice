...
func TestCheck(t *testing.T) {
	...
	validResponse := fdscommon.CheckResponse{
		Success: true,
		Data: fdscommon.CheckData{
			Tags: make([]fdscommon.CheckTags, 0),
		},
	}

	validRequest := fdscommon.CheckRequest{}
	...
	testCases := []struct {
		...
		expcResponse *fdscommon.CheckResponse
	}{
		{
		...
		{
			desc:    "success with mapped response",
			wantErr: false,
			mocker: func(m *Mocker) {
				m.cfg = mockConfig
				m.secret = mockSecret
