...
func TestAccountInquiriesRepository_GetByID(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name: "SUCCESS: Find Account by ID",
			mockSetup: func(mysqlMock *mysqlMocks.IMySqlExt) {
				mysqlMock.On(
					"GetContext",
			...
			id:        account.BeneficiaryBankCode,
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			userRes, err := repo.GetByID(ctx, tc.id)
