...
func TestUploadUnderlyingDocument(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:      "SUCCESS: upload document to xb core",
			wantError: false,
			setupMock: func(mockHttp *httpMocks.IHTTPRequest) {
				mockHttp.On(
					"POSTWithFormData",
					...
					constant.MapStrValPtrMultipartFileHeaderMockType(),
					constant.MapStrValStringMockType(),
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			_, err := repo.UploadUnderlyingDocument(context.Background(), &xbCoreProcessorModel.UploadUnderlyingDocumentRequest{
				XbPayoutId: uuid.NewString(),
			})
			if tc.wantError {
				assert.Error(t, err)
			} else {
