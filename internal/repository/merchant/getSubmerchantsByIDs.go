...
func (r *MerchantRepository) GetSubmerchantsByIDs(ctx context.Context, parentMerchantID string, submerchantIDs []string) ([]*merchant.Merchant, error) {
	...
	merchantIdsParam := strings.Join(submerchantIDs, "','")
	...
	query := `
		...
		FROM
			` + merchantsTable + ` as m
		WHERE m.uuid IN (%s) and m.parent_id = ?`
	query = fmt.Sprintf(query, merchantIdsParam)

	if err := r.db.SelectContext(ctx, &data, query, parentMerchantID); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			r.logger.Error(ctx, "submerchant not found", logger.Error(err), logger.Any("parentMerchantId", parentMerchantID), logger.Any("submerchantIds", submerchantIDs))
			return nil, nil
		}

		r.logger.Error(ctx, "error when finding submerchant", logger.Error(err), logger.Any("parentMerchantId", parentMerchantID), logger.Any("submerchantIds", submerchantIDs))
		return data, err
	}
