...
func TestUpdateBulkDisbursementStatusByID(t *testing.T) {
	testCase := []struct {
		name      string
	...
	for _, tc := range testCase {
		t.Run(tc.name, func(t *testing.T) {
			...
			err := repo.UpdateBulkDisbursementStatusByID(ctx, uuid.NewString(), constant.BulkDisbursementStatusWaiting)
			if tc.wantErr {
				assert.Error(t, err)
			} else {
