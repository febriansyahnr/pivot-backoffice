...
func TestCommitTransaction(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name: "Success commit",
			mockSetup: func(mysqlMock *mysqlMocks.IMySqlExt) {
				mysqlMock.On("Commit", ctx).Return(nil)
			},
			wantErr: false,
		},
		{
			name: "Fail commit",
			mockSetup: func(mysqlMock *mysqlMocks.IMySqlExt) {
				mysqlMock.On("Commit", ctx).Return(errors.New("commit error"))
...
func TestRollbackTransaction(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			mockSetup: func(mysqlMock *mysqlMocks.IMySqlExt) {
				mysqlMock.On("Rollback", ctx).Return(nil)
			},
			wantErr: false,
		},
		{
			name: "Fail rollback",
			mockSetup: func(mysqlMock *mysqlMocks.IMySqlExt) {
				mysqlMock.On("Rollback", ctx).Return(errors.New("rollback error"))
